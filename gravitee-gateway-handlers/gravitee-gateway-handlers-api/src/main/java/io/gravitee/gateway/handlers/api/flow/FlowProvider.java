...
public class FlowProvider {
    ...
    public StreamableProcessor<ExecutionContext, Buffer> provide(ExecutionContext context) {
        ...
        if (flows != null && !flows.isEmpty()) {
            ...
            for (Flow flow : flows) {
                //TODO: it could be better to not let the gateway to filter this. Maybe it can be done at the ApiManager
                // level to ensure that only relevant configuration are provided?
                if (flow.isEnabled()) {
                    // TODO: Check that the flow should be executed

                    chain.add(policyChainFactory.create(
